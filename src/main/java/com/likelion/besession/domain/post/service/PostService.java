...
@Transactional(readOnly = true)
    ...
    // 게시글 최신순으로 조회
    @Transactional(readOnly = true)
    public List<PostResponse> createdAtDescPost() {

        // 1. 게시글 최신순으로 불러오기
        List<Post> post = postRepository.findAllByOrderByCreatedAtDesc();

        // 2. 반환하기
        return post.stream().map(this::toPostResponse).toList();
    }

    // 게시글 조회수 많은 순으로 조회
    @Transactional(readOnly = true)
    public List<PostResponse> moreViewDescPost() {

        // 1. List<PostResponse> 객체 생성
        List<PostResponse> postResponses = new ArrayList<>();

        // 2. 게시글 조회순 많은 순서대로 불러오기
