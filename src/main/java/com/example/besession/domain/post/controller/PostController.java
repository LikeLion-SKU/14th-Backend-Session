package com.example.besession.domain.post.controller;
...
public class PostController {
    ...
    @Operation(summary="게시글 생성", description = "요청으로 전달된 게시글 정보롤 새로운 게시물을 생성하는 API")
    @PostMapping("/posts")
...
/*    @Operation(summary="게시글 전체 조회",description = "모든 게시글 목록을 조회하는 API")
    ...
    public ResponseEntity<List<PostResponse>> getAllPosts() {
        ...
        List<PostResponse> response = postService.getAllPosts2();
...
@Operation(summary = "게시글 단건 조회",description = "게시글 ID로 특정 게시글 조회하는 API")
    ...
    public ResponseEntity<PostResponse> updatePost(@PathVariable("post-id") Long postId, @RequestBody UpdatePostRequest request) {
